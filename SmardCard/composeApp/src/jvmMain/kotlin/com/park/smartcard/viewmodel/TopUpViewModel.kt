package com.park.smartcard.viewmodel

import androidx.lifecycle.ViewModel
import androidx.lifecycle.viewModelScope
import kotlinx.coroutines.flow.MutableStateFlow
import kotlinx.coroutines.flow.StateFlow
import kotlinx.coroutines.launch
import com.park.smartcard.data.model.CardDTO
import com.park.smartcard.data.model.CustomerDTO
import com.park.smartcard.data.model.TopUpResult
import com.park.smartcard.data.repository.CardRepository
import com.park.smartcard.data.repository.CustomerRepository
import com.park.smartcard.data.repository.GamePlaySyncRepository
import com.park.smartcard.data.repository.NfcCardRepository
import com.park.smartcard.data.repository.PendingGamePlayRepository
import com.park.smartcard.data.repository.isAuthError
import java.math.BigDecimal

enum class StaffTopUpMethod {
    CASH,
    MOMO
}

data class TopUpState(
    val isReading: Boolean = false,
    val isFetching: Boolean = false,
    val isTopingUp: Boolean = false,
    val isPollingMomo: Boolean = false,
    val selectedMethod: StaffTopUpMethod = StaffTopUpMethod.CASH,
    val scannedCard: CardDTO? = null,
    val customer: CustomerDTO? = null,
    val momoPayment: TopUpResult? = null,
    val momoStatusMessage: String? = null,
    val successMessage: String? = null,
    val errorMessage: String? = null
)

class TopUpViewModel(
    private val customerRepo: CustomerRepository = CustomerRepository(),
    private val cardRepo: CardRepository = CardRepository(),
    private val gamePlaySyncRepo: GamePlaySyncRepository = GamePlaySyncRepository(),
    private val nfcRepo: NfcCardRepository = NfcCardRepository(),
    private val pendingRepository: PendingGamePlayRepository = PendingGamePlayRepository()
) : ViewModel() {

    private val _state = MutableStateFlow(TopUpState())
    val state: StateFlow<TopUpState> = _state

    private val momoPoller = MomoStatusPoller(
        scope = viewModelScope,
        getStatus = { userId, orderId -> customerRepo.getTopUpStatus(userId, orderId) }
    )

    fun scanCard() {
        viewModelScope.launch {
            _state.value = _state.value.copy(isReading = true, errorMessage = null)

            val flushResult = flushPendingTransactions()
            if (flushResult.isFailure) {
                _state.value = _state.value.copy(
                    isReading = false,
                    errorMessage = flushResult.exceptionOrNull()?.message ?: "Không thể đồng bộ lượt chơi chờ."
                )
                return@launch
            }

            nfcRepo.readCardId().fold(
                onSuccess = { cardId -> onCardDetected(cardId) },
                onFailure = {
                    _state.value = _state.value.copy(isReading = false, errorMessage = it.message)
                }
            )
        }
    }

    private suspend fun onCardDetected(cardId: String) {
        _state.value = _state.value.copy(isReading = false, isFetching = true)
        cardRepo.lookupCardByCardId(cardId).fold(
            onSuccess = { card ->
                val userId = card.userId
                if (userId == null) {
                    _state.value = _state.value.copy(
                        isFetching = false,
                        errorMessage = "Thẻ chưa liên kết với tài khoản nào"
                    )
                    return
                }

                customerRepo.getCustomerById(userId).fold(
                    onSuccess = { customer ->
                        _state.value = _state.value.copy(
                            isFetching = false,
                            scannedCard = card,
                            customer = customer
                        )
                    },
                    onFailure = { e ->
                        _state.value = _state.value.copy(isFetching = false, errorMessage = e.message)
                    }
                )
            },
            onFailure = { e ->
                _state.value = _state.value.copy(isFetching = false, errorMessage = e.message)
            }
        )
    }

    fun selectMethod(method: StaffTopUpMethod) {
        if (_state.value.selectedMethod == method) return
        momoPoller.cancel()
        _state.value = _state.value.copy(
            selectedMethod = method,
            isPollingMomo = false,
            momoPayment = null,
            momoStatusMessage = null,
            errorMessage = null,
            successMessage = null
        )
    }

    fun topUp(amount: String) {
        when (_state.value.selectedMethod) {
            StaffTopUpMethod.CASH -> topUpCash(amount)
            StaffTopUpMethod.MOMO -> createMomoTopUp(amount)
        }
    }

    private fun topUpCash(amount: String) {
        val scannedCard = _state.value.scannedCard ?: return
        val customer = _state.value.customer ?: return

        val topUpAmount = amount.toBigDecimalOrNull()
        if (topUpAmount == null || topUpAmount <= BigDecimal.ZERO) {
            _state.value = _state.value.copy(errorMessage = "Số tiền không hợp lệ")
            return
        }

        viewModelScope.launch {
            _state.value = _state.value.copy(isTopingUp = true, errorMessage = null)

            val flushResult = flushPendingTransactions()
            if (flushResult.isFailure) {
                _state.value = _state.value.copy(
                    isTopingUp = false,
                    errorMessage = flushResult.exceptionOrNull()?.message ?: "Không thể đồng bộ lượt chơi chờ."
                )
                return@launch
            }

            val latestCustomer = customerRepo.getCustomerById(customer.userId).getOrElse { error ->
                _state.value = _state.value.copy(isTopingUp = false, errorMessage = error.message)
                return@launch
            }

            val serverBalanceBefore = latestCustomer.currentBalance.toBalanceInt()

            nfcRepo.applyCashTopup(
                expectedCardId = scannedCard.cardId,
                serverBalanceBefore = serverBalanceBefore,
                doServerTopup = {
                    customerRepo.topUpForCustomer(customer.userId, amount, "CASH").map { result ->
                        result.currentBalanceAfter?.toBalanceInt()
                            ?: serverBalanceBefore + topUpAmount.toInt()
                    }
                }
            ).fold(
                onSuccess = { newBalance ->
                    _state.value = _state.value.copy(
                        isTopingUp = false,
                        customer = latestCustomer.copy(currentBalance = newBalance.toString()),
                        successMessage = "Nạp tiền thành công và đã cập nhật số dư trên thẻ"
                    )
                },
                onFailure = { error ->
                    _state.value = _state.value.copy(isTopingUp = false, errorMessage = error.message)
                }
            )
        }
    }

    private fun createMomoTopUp(amount: String) {
        val customer = _state.value.customer ?: return

        val topUpAmount = amount.toBigDecimalOrNull()
        if (topUpAmount == null || topUpAmount <= BigDecimal.ZERO) {
            _state.value = _state.value.copy(errorMessage = "Số tiền không hợp lệ")
            return
        }

        momoPoller.cancel()
        viewModelScope.launch {
            _state.value = _state.value.copy(
                isTopingUp = true,
                isPollingMomo = false,
                momoPayment = null,
                momoStatusMessage = null,
                errorMessage = null,
                successMessage = null
            )

            val flushResult = flushPendingTransactions()
            if (flushResult.isFailure) {
                _state.value = _state.value.copy(
                    isTopingUp = false,
                    errorMessage = flushResult.exceptionOrNull()?.message ?: "Không thể đồng bộ lượt chơi chờ."
                )
                return@launch
            }

            customerRepo.topUpForCustomer(customer.userId, amount, "MOMO").fold(
                onSuccess = { payment ->
                    val orderId = payment.orderId
                    _state.value = _state.value.copy(
                        isTopingUp = false,
                        isPollingMomo = !orderId.isNullOrBlank(),
                        momoPayment = payment,
                        momoStatusMessage = "Đang chờ khách quét QR và thanh toán MoMo"
                    )

                    if (orderId.isNullOrBlank()) {
                        _state.value = _state.value.copy(
                            isPollingMomo = false,
                            errorMessage = "Không nhận được mã giao dịch MoMo"
                        )
                    } else {
                        momoPoller.start(
                            userId = customer.userId,
                            orderId = orderId,
                            initialPayment = payment,
                            onEvent = ::handleMomoEvent
                        )
                    }
                },
                onFailure = { error ->
                    _state.value = _state.value.copy(isTopingUp = false, errorMessage = error.message)
                }
            )
        }
    }

    fun checkMomoStatusNow() {
        val customer = _state.value.customer ?: return
        val orderId = _state.value.momoPayment?.orderId ?: return
        viewModelScope.launch {
            momoPoller.checkOnce(customer.userId, orderId, ::handleMomoEvent)
        }
    }

    fun cancelMomoTopUp() {
        momoPoller.cancel()
        _state.value = _state.value.copy(
            isPollingMomo = false,
            momoPayment = null,
            momoStatusMessage = null
        )
    }

    /** Map từ event của poller sang UiState. Trả `true` nếu muốn poller dừng sớm. */
    private suspend fun handleMomoEvent(event: MomoStatusPoller.Event): Boolean {
        return when (event) {
            is MomoStatusPoller.Event.Pending -> {
                _state.value = _state.value.copy(
                    isPollingMomo = true,
                    momoPayment = event.payment,
                    momoStatusMessage = "Đang chờ khách thanh toán MoMo"
                )
                false
            }
            is MomoStatusPoller.Event.Success -> {
                val userId = _state.value.customer?.userId
                if (userId != null) {
                    syncCardAfterMomoSuccess(userId, event.payment)
                }
                false
            }
            is MomoStatusPoller.Event.Failed -> {
                _state.value = _state.value.copy(
                    isPollingMomo = false,
                    momoPayment = event.payment,
                    momoStatusMessage = null,
                    errorMessage = "Giao dịch MoMo không thành công"
                )
                false
            }
            MomoStatusPoller.Event.Timeout -> {
                _state.value = _state.value.copy(
                    isPollingMomo = false,
                    momoStatusMessage = "Giao dịch vẫn đang chờ xử lý. Có thể bấm kiểm tra lại sau."
                )
                false
            }
            is MomoStatusPoller.Event.StatusError -> {
                _state.value = _state.value.copy(
                    momoStatusMessage = event.message
                )
                false
            }
        }
    }

    private suspend fun syncCardAfterMomoSuccess(userId: String, payment: TopUpResult) {
        val scannedCard = _state.value.scannedCard
        val latestCustomer = customerRepo.getCustomerById(userId).getOrElse { error ->
            _state.value = _state.value.copy(
                isPollingMomo = false,
                momoPayment = payment,
                errorMessage = "MoMo đã thanh toán nhưng không tải được số dư mới: ${error.message}"
            )
            return
        }

        if (scannedCard == null) {
            _state.value = _state.value.copy(
                isPollingMomo = false,
                momoPayment = payment,
                errorMessage = "Không còn thông tin thẻ vừa quẹt."
            )
            return
        }

        val newBalance = latestCustomer.currentBalance.toBalanceInt()
        nfcRepo.syncBalanceToCard(
            expectedCardId = scannedCard.cardId,
            newBalance = newBalance
        ).fold(
            onSuccess = {
                _state.value = _state.value.copy(
                    isPollingMomo = false,
                    momoPayment = null,
                    customer = latestCustomer,
                    momoStatusMessage = null,
                    successMessage = "MoMo thanh toán thành công và đã cập nhật số dư trên thẻ"
                )
            },
            onFailure = { error ->
                _state.value = _state.value.copy(
                    isPollingMomo = false,
                    momoPayment = payment,
                    momoStatusMessage = null,
                    errorMessage = "MoMo đã thanh toán nhưng ${error.message}"
                )
            }
        )
    }

    fun reset() {
        momoPoller.cancel()
        _state.value = TopUpState()
    }

    fun clearMessages() {
        _state.value = _state.value.copy(successMessage = null, errorMessage = null)
    }

    private suspend fun flushPendingTransactions(): Result<Unit> {
        val result = pendingRepository.flush { play -> gamePlaySyncRepo.syncPendingGamePlay(play) }
        if (result.failure != null) {
            val error = result.failure
            val prefix = if (result.syncedCount > 0) {
                "Đã đồng bộ ${result.syncedCount} giao dịch, "
            } else {
                ""
            }
            val message = when {
                error.isAuthError() -> prefix + "phiên đăng nhập không còn hợp lệ."
                else -> prefix + (error?.message ?: "không thể đồng bộ giao dịch chờ.")
            }
            return Result.failure(Exception(message, error))
        }
        return Result.success(Unit)
    }

    override fun onCleared() {
        momoPoller.cancel()
        super.onCleared()
    }
}

private fun String.toBalanceInt(): Int {
    return toBigDecimalOrNull()?.setScale(0)?.toInt() ?: 0
}
